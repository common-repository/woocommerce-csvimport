...
class woocsv_import_admin {
	...
	public function delete_header() {
		...
		if ( isset ( $header_name ) ) {
			unset ( $woocsv_import->headers[ $header_name ] );
	...
	public function import() {
		...
		if ( isset ( $_FILES[ 'csvfile' ] ) && $_FILES[ 'csvfile' ][ 'error' ] != 0 ) {
			...
			echo sprintf( __( 'Something went wrong during the file upload, error message: %s', 'woocommerce-csvimport' ),
				UploadErrorMessages::convertErrorToMessage( $_FILES[ 'csvfile' ][ 'error' ] ) );
			echo '</p></div>';
		}
		...
		//preview the header
		if ( isset( $_POST[ 'action' ] ) && $_POST[ 'action' ] === 'start_header_preview' && @is_uploaded_file( $_FILES[ 'csvfile' ][ 'tmp_name' ] ) && check_admin_referer( 'upload_header_file',
				'upload_header_file' ) ) {
		...
		/* ! IMPORT */
		//preview the import
		if ( isset( $_POST[ 'action' ] ) && $_POST[ 'action' ] === 'start_import_preview' && @is_uploaded_file( $_FILES[ 'csvfile' ][ 'tmp_name' ] ) && check_admin_referer( 'upload_import_file',
				'upload_import_file' ) ) {
//			@since 3.1.0
			//@todo create batch allready!
			//set the active header
			$header_name = sanitize_text_field( $_POST[ 'header_name' ] );
			$header      = $woocsv_import->headers[ $header_name ];
			update_option( 'woocsv_header', $header );
	...
	public static function save_header() {
		...
		foreach ( $_POST as $key => $value ) {
			if ( preg_match( "/fields_[0-9]/", $key, $matches ) ) {
				$headerOrder[] = sanitize_text_field( $value );
			}
		}

		$headers                 = $woocsv_import->headers;
		$header_name             = sanitize_text_field( $_POST[ 'header_name' ] );
		$headers[ $header_name ] = $headerOrder;
